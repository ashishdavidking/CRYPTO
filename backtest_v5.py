"""
Backtesting Engine for alpha_bot_v5
Timeframe: 15-Minute | Strategy: Fibonacci Reversal & Breakthrough
"""
...
# Config
SYMBOL = "BTCUSD"
TIMEFRAME_SECS = 900  # 15 mins
ATR_PERIOD = 10
ST_FACTOR = 4.5
CONFIRM_BARS = 3
DOJI_FACTOR = 0.1
MAX_STATE_AGE_BARS = 20
...
def _get_headers(method: str, path: str, query_string: str = "") -> dict:
    ...
    return {
        ...
        "User-Agent": "python-alpha-backtest",
...
# ═══════════════════════════════════════════════════════════════
def fetch_historical_candles_chunked(days: int = 90) -> list:
    cache_file = f"btc_15m_hist_{days}d.json"
    if os.path.exists(cache_file):
        console.print(f"[green]Loading historical data from cache file: {cache_file}[/green]")
        with open(cache_file, "r") as f:
            return json.load(f)

    console.print(f"[cyan]Fetching {days} days of historical 15m candles in chunks...[/cyan]")
    ...
    # 500 candles per chunk = 500 * 15 minutes = 7500 minutes = 125 hours (~5.2 days)
...
def run_backtest(candles: list):
    ...
    for i in range(1, len(candles)):
        # Supertrend result index mapping
        ...
        # 1. Manage Active Position (SL/TP check)
        if position:
            side = position["side"]
            ...
            if hit_sl or hit_tp:
                ...
                exit_price = sl if hit_sl else tp  # conservative execution
        ...
        # 2. Check Pending Activation
            ...
            if triggered:
                ...
                # Check if position gets stopped out / hit TP in the exact same candle
                sl = position["sl_price"]
                tp = position["tp_price"]
                hit_sl = (side == "buy" and p_low <= sl) or (side == "sell" and p_high >= sl)
                hit_tp = (side == "buy" and p_high >= tp) or (side == "sell" and p_low <= tp)
        ...
        # 3. Supertrend & Phase Logic updates (Runs on Candle Close)
        ...
        # 4. Check for Trade Signals on Candle Close
...
if __name__ == "__main__":
    candles = fetch_historical_candles_chunked(days=90)
    if not candles:
    ...
    # Format tables
    trade_table = Table(title="Historical Executed Trades (Last 90 Days)", header_style="bold cyan")
    ...
    console.print("\n")
    console.print(Panel(metric_table, title="[bold yellow]BACKTEST PERFORMANCE SUMMARY[/bold yellow]", border_style="yellow"))
